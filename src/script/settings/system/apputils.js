import { saveSettings, getSettings, exportSettings, importSettings, subscribe } from "/src/script/core/storagehandler.js";
import { getFormattedClock as coreGetFormattedClock, initDate } from "/src/script/core/time.js";
import { openCustomPopup, showNotification, createConfirmDialog } from "/src/script/core/UI.js";
    ...
    if (resetSettingsBtn) {
        resetSettingsBtn.addEventListener("mousedown", () => {
            const dialogData = createConfirmDialog(
                ...
                async () => {
                    const { clearStore } = await import("/src/script/core/db.js");
                    await clearStore();
                    localStorage.clear();
                    setTimeout(() => {
                        location.reload();
