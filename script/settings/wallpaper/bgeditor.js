...
class BackgroundEditor {
     ...
     */
    open() {
        const bgUrl = getComputedStyle(this.realLayer).backgroundImage;
        if (!bgUrl || bgUrl === "none") {
    ...
    setupSliders() {
        ...
        const specs = [
            {
                id: "zoom", label: t("bg_editor.zoom_label"),
    ...
    loadImageAndCalculate(bgUrl) {
        ...
        imgObj.onload = () => {
            ...
            //
            // Mirrors what CSS background-size: cover does on the real screen:
            //
            //  If screenRatio > imgRatio  →  image scales to fill width:
            //    bgW = viewportW (no x-overflow), bgH > viewportH (y-overflow)
            //    ∴ baseLensW = viewW  (fills editor width)
            //      baseLensH = viewW / screenRatio
            //    Proof: bgW/viewportW = 1 = viewW/baseLensW  ✓
            //
            //  If screenRatio ≤ imgRatio  →  image scales to fill height:
            //    bgH = viewportH (no y-overflow), bgW > viewportW (x-overflow)
            //    ∴ baseLensH = viewH  (fills editor height)
            //      baseLensW = viewH * screenRatio
            //    Proof: bgW/viewportW = imgRatio/screenRatio = viewW/baseLensW  ✓
